...
| 重复消息与服务重启 | `message_id` 业务契约持久化到 SQLite：**类型与规范化载荷完全相同**的消息任意次投递（含重启后、并发）只生效一次并返回首次结果（`outcome:"replay"`）；**类型或载荷不同**判为编号复用冲突，返回 HTTP 409 拒绝执行（无任何资源分配），首次与冲突摘要写入冲突台账 |
| 冲突交接追踪 | `GET /internal/conflicts` 回看每次 message_id 复用冲突：首次命令类型/载荷摘要、冲突类型/载荷摘要、检测时间 |
...
| GET | `/internal/conflicts` | 消息编号复用冲突台账（首次/冲突摘要） |
...
```bash
npm test          # python3 -m unittest discover，共 58 个用例
python3 -m compileall -q .  # 编译检查全部 Python 模块
```

覆盖：诊断字段拒绝与公众脱敏、能力/资质/期限匹配、同车双承诺禁止与容量拼车、三类增量重排只动受影响段、紧急占用理由+通知+原段重排、并发与重启幂等契约（相同消息重放/改字段或跨类型复用即 409 冲突且无部分分配）、跨城交接成立条件与断档检测、HTTP 契约。
